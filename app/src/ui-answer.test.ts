...
describe('answer screen', () => {
  it('lights the held slots and dims the rest', () => {
    ...
    renderAnswer(root, GAMES.ddb, R(12, 11, 10, 5, 1), [null,null,null,null,null],
  ...
  it('never names a rank in the instruction, only positions', () => {
    const root = document.createElement('div');
    renderAnswer(root, GAMES.ddb, R(5, 5, 11, 10, 9), [0, 1, 0, 0, 0],
  ...
  it('names the active game so an answer cannot be read as the other one', () => {
    const root = document.createElement('div');
    renderAnswer(root, GAMES.deuces, R(12, 11, 10, 5, 1), [null,null,null,null,null],
      entryMask(0b00111), 2.5, 2.5, vi.fn());
