...
fn main() {
    // Canonical hands, orbit sizes and the cross-language fixture do not
    // depend on the paytable, so they are built once and shared.
    ...
    sweep(&hands, "9/6 Double Double Bonus", "out/ddb96.bin",
          eval::ddb::payout, "98.98%");
    sweep(&hands, "Illinois Deuces Wild", "out/illinois-deuces.bin",
          eval::deuces::payout, "98.91%");
}

fn sweep(
    hands: &[([card::Card; 5], usize)],
    label: &str,
    path: &str,
    payout: Payout,
    published: &str,
    ...
    let solved: Vec<(usize, u8, f64, u32)> = hands
        .par_iter()
        .map(|(hand, idx)| {
            let best = solve::solve(hand, payout);
    ...
    println!("  (published figure is ~{}. If the self-proving checks all pass", published);
    println!("   and this disagrees, investigate the published figure.)");
